...
if archivo_subido is not None:
    try:
        ...
        # Lista exhaustiva de todos los términos que significan pendiente
        ...
        # Aplicamos la lista robusta para encontrar a los pendientes
        ...
        if total_colaboradores == 0:
            ...
            # --- 🚀 NUEVA LÓGICA: AGRUPACIÓN POR PUESTOS ---
            def categorizar_puesto(puesto):
                p_lower = str(puesto).lower()
                if p_lower == '---':
                    return 'Z_Recesos' # Manda los recesos operativos hasta el final del documento
                    ...
                    # Si hay algún puesto extra (ej. Prevención), lo agrupa bajo su propio nombre
                    return f"E_{str(puesto).title()}"
            
            # Creamos una columna temporal oculta para darle el orden maestro
            df_horarios['Categoria_Orden'] = df_horarios['Puesto'].apply(categorizar_puesto)
            # Ordenamos: 1ro por Área (Gerencia, Cajas...), 2do por Colaborador, 3ro por Horario
            df_horarios = df_horarios.sort_values(by=['Categoria_Orden', 'Colaborador', 'Fecha y Horario']).reset_index(drop=True)
            # Borramos la columna temporal para que el formato de 5 columnas del Excel siga funcionando perfecto
            df_horarios = df_horarios.drop(columns=['Categoria_Orden'])

            # --- ESTILOS DE COLORES POR DÍA ---
            ...
            # --- 🎨 PROCESO EXCEL EXCLUSIVE OPENPYXL ---
            output = io.BytesIO()
            with pd.ExcelWriter(output, engine='openpyxl') as writer:
                ...
                # 🔴 TABLA LATERAL: TOP 10 MAYOR REZAGO
                ...
                # 🟢 TABLA LATERAL: TOP 10 MEJOR APROVECHAMIENTO
                ...
                for i, (idx, fila) in enumerate(top_10_mejor.iterrows(), start=19):
                    worksheet.cell(row=i, column=7).fill = fill_lista_mejor
                    worksheet.cell(row=i, column=7).font = font_lista_mejor
                    worksheet.cell(row=i, column=7).border = borde
                    worksheet.cell(row=i, column=7).alignment = Alignment(vertical="center")
                    
                    worksheet.cell(row=i, column=8).fill = fill_lista_mejor
                    worksheet.cell(row=i, column=8).font = font_lista_mejor
                    worksheet.cell(row=i, column=8).border = borde
                    worksheet.cell(row=i, column=8).alignment = Alignment(horizontal="center", vertical="center")
